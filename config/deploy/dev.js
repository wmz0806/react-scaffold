...
const zipName = `${pkg.name}.zip`; // zip 压缩后的临时文件名
const pm2Cmd = '/usr/local/node/bin/pm2'; // 服务器pm2命令位置

const {
  build = 'cross-env NODE_ENV=production webpack --config config/webpack.config.prod.js', // 打包命令
  host = '10.10.100.2',
  remotePath = '/home/tomcat/node/项目名字', // 服务器文件目录
  pmApp = '项目名字', // pm2本项目的 app名字
} = argv; // pakage.json script 传入的参数

// 测试环境
const USER = 'root'; // 用户名
const pw = 'G3Sde1d2'; // 密码
...
async function main() {
  ...
  await scp();
  // exec(`rm ${path.join(process.cwd(), zipName)}`);
